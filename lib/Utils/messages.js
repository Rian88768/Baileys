...
const prepareWAMessageMedia = async (message, options) => {
    ...
    // --- START PATCH: sanitize annotations to remove any newsletter metadata ---
    const sanitizedAnnotations = (message.annotations && Array.isArray(message.annotations))
        ? message.annotations.map(a => {
            // shallow copy to avoid mutating original
            const copy = Object.assign({}, a);
            if (copy.newsletter) delete copy.newsletter;
            return copy;
        })
        : [
            {
    ...
    const uploadData = {
        ...message,
        ...(sanitizedAnnotations && sanitizedAnnotations.length ? { annotations: sanitizedAnnotations } : {}),
